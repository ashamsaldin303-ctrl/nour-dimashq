...
Task ID: T2.1
Agent: Z.ai Code (main orchestrator)
Task: Rework the M2 cinema layer after the user rejected the result ("الفيديو مقلوب + السكرول يتحرك صورة صورة + الهيكلية على الفيديو، وليس الفيديو على الهيكلية") — precise diagnosis + full-film scrub rebuild.

Work Log:
- DIAGNOSIS (evidence-backed): (1) THE FLIP — keyframe-engine.ts uploaded textures with UNPACK_FLIP_Y_WEBGL=false while the verbatim §8.2 shader maps clip-space bottom to UV 0 → every WebGL canvas rendered vertically flipped; VLM confirmed on the live site ("fountain and floor at the top, ceiling at the bottom") while the source video itself probes correct (no rotation metadata; VLM on extracted frames: "correct/correct"). (2) THE SLIDESHOW — Tier A was 4 static keyframes + crossfade/parallax (a photo-stepper by construction); Tier B video was desktop-only + env-gated + door-gated, split into 3 intra clips, and its scrubber throttled seeks to 80ms + 1/24s commit (≤12 updates/s) — mobile never saw video at all. (3) THE STRUCTURE — the video had been chopped to fit the pre-existing 3-chapter layout; the user ordered the inverse.
- ARCHITECTURE PIVOT (user ruling): ONE full continuous film as the structural spine. public/media/film/film.mp4 = H.264 720p60 GOP30 faststart silent 4.35MB (source HEVC never plays in Chrome/Firefox — normalized), poster.webp = frame 0 @1600w (seamless pre-video paint). Manifest v2: src/poster/durationSec/sizeMb/runwayVh + caption time-windows; MEASURED numbers only; honest-bytes HEAD guard (±20%); loader cache fixed to no-cache (force-cache served a stale v1 manifest from the browser cache — found+fixed live).
- NEW ENGINE (film-scrub-engine.ts): native <video> element (browser-composited → orientation ALWAYS correct, the flip bug class is eliminated) mounted in a sticky stage; damped time landing (τ=70ms) on the ONE gsap ticker; at most one seek per completed seek-cycle ('seeked' re-arms, 250ms stale guard) at up to every-frame cadence; fastSeek only for ≥1.5s jumps; iOS LPM first-gesture recovery; onFirstFrame gates the CSS fade-in over the same-frame poster.
- STRUCTURE ON THE VIDEO: prologue poster (100svh) → sticky stage + 560svh scroll runway (scroll progress = film time 0→11.87s, measured live: 0.06/1.47/2.89/4.31/5.73/7.15/8.56/9.98/11.40/11.87 at even scroll steps) → captions العتبة/الباح/النور as reversible time-windowed overlays (CSS class toggles + per-word staggered transitions, scrub-safe both directions) → match-cut into /#properties. No pin spacers (CSS sticky), no WebGL, no keyframes, no depth maps, no clip chain.
- CONSENT MODEL: desktop auto-runs the film (the cinema IS the site); touch/<1024 or Save-Data gets the honest-MB door first (label computed from film.sizeMb → «٤٫٣ ميغابايت»); «إيقاف السينما» demotes to the poster rung and persists; legacy storage key honored; reduced-motion → statics storyboard (poster + 3 captions in flow, zero engine); every failure fail-closes to the poster rung (M1 hero intact beneath).
- DELETED: keyframe-engine.ts, scrub-video-engine.ts, scene-engine.ts, progress-store.ts, cinema-chapter.tsx, keyframes/ depth/ derivatives/ assets (−3.7MB clips −260KB stills), depth-gen/budget-check pipeline scripts, NEXT_PUBLIC_CINEMA_TIER_B env flag. KEPT: spine.ts (one clock), cinema-prologue.tsx, cinema-film.tsx (ssr:false wrapper), poster-hero mount.
- ADDED: scripts/cinema-pipeline/build-film.ts (reproducible: encode + poster + manifest emit with measured numbers) + README.
- VERIFIED (agent-browser + VLM, evidence in tool-results/m2-audit/): orientation correct ×4 VLM checks ("right side up" on stage frames mid-film + near-end); scrub linear + reversible; captions toggle exactly in windows (ataba@0.3-3.7 / baha@4.0-7.7 / nour@8.1-11.6); sticky stage holds top:0 through the runway and releases at the end; mobile 375 fresh = zero video bytes before consent + door + accept → live scrub, no horizontal scroll @375/768/1440; reduced-motion statics (3 items, no stage/video); off-toggle frees the decoder + poster rung + persists; console clean; match-cut + #properties handoff intact; tsc 0 / eslint 0.

Stage Summary:
- The two reported defects are eliminated AT THE ROOT (native video = no flip surface; one continuous film = no slideshow), and the structure now sits ON the video per the user's ruling: scroll = the film's timeline, captions = time windows, funnel = after the match-cut.
- Film budget: 4.35MB honest label, zero bytes pre-consent on touch/Save-Data, desktop auto-on per owner intent; full fallback ladder preserved.
- Files: +film-scrub-engine.ts +build-film.ts, ~manifest.ts ~fallback.ts ~film.tsx ~cinema-consent.tsx ~globals.css ~cinema.json ~.env, −5 dead modules −3 asset dirs. Ready for GitHub push.
